...
- Two run types were used: multibar and werdering. The first, termed ‘multibar’, involves bars sweeping in multiple directions (same as RETBAR in the Human Connectome Project 7T Retinotopy Dataset). The second, termed ‘wedgering’, involves a combination of rotating wedges and expanding and contracting rings.
...
### Motor Localizer
- 1 run for the 2nd localizer session and during final cued recall session

### Auditory Category Localizer
- 1 run for the 1st localizer session and during final cued recall session
...
### Encoding
- Encoding Task: “How well are you able to associate the word with the image?” Select 1 if it was not at all, 2 for somewhat, and 3 for very well.
...
### Cued Recall
- Recall Task: “Please retrieve the image/word associated with the word/image cue as vividly as possible, and rate the vividness of your memory.” Select 1 for least vivid, 2 for some-what vivid, and 3 for most vivid
...
- Participant had 4 runs of a final cued recall session, the same as in the trial-based task, in which the participants was given either the word or image cue, and were instructed to recall the image or word that they saw during the 15 previous cued recall scans
...
### Movie Viewing
- Watched 8 2-6 minute videos
- Types of movies: Length and style (live-action, animated, stop motion, with or without speech)
- Conditions: 6 of the movies are new and 2 are repeated every session
- Participants are first shown the movie title for 2.5 seconds and then are shown the movie
### Free Recall
- Participants are prompted with a movie title for 7.5 seconds. During this time, the participant indicates if they are able to recall the movie that is associated with the movie title. 
- If they are unable to recall the movie, they are then shown an image cue for 3 seconds. 
- A fixation cross is then shown, and the participant speaks freely into a microphone for an unlimited amount of time for them to be able to recall as many details as possible from the movie. 
...
## Resting State Task
- Focused on a fixation point for 5 mins
